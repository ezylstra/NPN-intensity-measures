...
# Make intensity midpoints = 0 if status = 0 and add intensity cat labels
si <- si %>%
...
si %>%
  group_by(common_name, phenophase_description, intensity_label) %>% 
  summarize(n_obs = n(),
            n_inphase = sum(phenophase_status),
            prop_inphase = round(n_inphase / n_obs, 2)) %>%
  data.frame() %>%
  arrange(intensity_label, desc(prop_inphase))
...
# for (intens in intensity_cats$intensity_label) {
  ...
  # for (spp in spps) {
    ...
    ggplot(si_int_spp, aes(x = day_of_year, y = intensity_midpoint)) +
