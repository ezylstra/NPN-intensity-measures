...
for (i in 1:nrow(counts)) {
  ...
  for (spp in spps) {
    ...
    # Annual smooths look very similar to those from GS model
    ...
    # proportion of plants in phase?
      ...
      # }
      # 
      # preds_prop <- plot_predictions(m_prop, by = "doy", draw = FALSE,
      #                                type = "link", transform = invlogit) %>%
      #   select(doy, estimate, conf.low, conf.high) %>%
      #   rename(day_of_year = doy,
      #          fitted = estimate,
      #          lwr_ci = conf.low,
      #          upr_ci = conf.high)
      # 
      # p_prop <- ggplot(data = preds_prop, aes(x = day_of_year, y = fitted)) +
...
i = 1 # 5
  ...
  spp <- spps[1]
    ...
    # Aggregate data across individual plants and years
    props_agg <- intdf %>%
    ...
    # Model with only a global smooth ("G" model in Pedersen)
    m_g <- glmmTMB(prop ~ s(day_of_year, k = kval, bs = cubic_bs), 
                   weights = n_obs,
                   data = props_agg, REML = TRUE, family = ordbeta)
    summary(m_g)
    ...
    # model but the CIs are nonsensically huge)
    newdata_g <- data.frame(
      day_of_year = seq(min(props_agg$day_of_year), max(props_agg$day_of_year)),
      n_obs = NA)
    ...
    for (yr in sort(unique(intdf$yr))) {
      ...
      doys <- seq(min_doy, max_doy)
      if (yr == min(intdf$yr)) {
        newdata_gs <- data.frame(fyr = yr, day_of_year = doys)
      } else {
        newdata_gs <- rbind(newdata_gs, data.frame(fyr = yr, day_of_year = doys))
      }
    }
    newdata_gs$fyr <- factor(newdata_gs$fyr)
    newdata_gs$n_indiv <- NA
    
    # Make annual predictions (averaging over individual plant effects)
    preds_gs <- predict(m_gs, newdata_gs, type = "link", se.fit = TRUE)
    preds_gs <- cbind(newdata_gs, preds_gs)
    preds_gs <- preds_gs %>%
      mutate(lwr_ci = ilink(fit - (2 * se.fit)),
             upr_ci = ilink(fit + (2 * se.fit)),
             fitted = ilink(fit))
    ...
    ggplot(data = preds_gs, aes(x = day_of_year)) +
      facet_wrap(~fyr) +
      geom_ribbon(aes(ymin = lwr_ci, ymax = upr_ci), alpha = 0.25) +
      geom_line(aes(y = fitted)) +
      geom_point(data = props_yragg, aes(y = prop, size = n_indiv), 
