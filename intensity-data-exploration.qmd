...
combos <- distinct(gamdf, intensity_label, intensity_type, common_name)
...
intdf <- intdf %>%
  mutate(Time = as.numeric(as.Date(observation_date))) %>%
  mutate(TimeC = Time - median(Time))

intdf1 <- intdf %>%
  filter(individual_id == 361264)

# GAM fit to one plant's data
trend1 <- gam(intensity_midpoint_log ~ s(day_of_year, bs = "cc", k = 20) + s(TimeC),
              data = intdf1)
notrend1 <- gam(intensity_midpoint_log ~ s(day_of_year, bs = "cc", k = 20),
                data = intdf1)
...
plot(notrend1)
# Trend really doesn't matter much

# GAM fit to multiple plants' data
intdf$individual_id <- factor(intdf$individual_id)
intdf$yr <- factor(intdf$yr)
# Fit different curve each year, intercepts vary among plants
m1 <- gam(intensity_midpoint_log ~ 
            individual_id + yr + s(day_of_year, bs = "cc", k = 10, by = yr),
          knots = list(day_of_year = c(105, 277)), 
          method = "REML", data = intdf)
summary(m1)

visreg(m1, xvar = "day_of_year",
       by = "yr", data = intdf,
       method = "REML")

# Predictions extend curve below values that are known minimums. Not sure how
# to avoid this, but could look into:
  # only make predictions between min/max date in each year
  # adaptive smooths
  # different smoothing functions
  # tried m = 1 (adjusting the penalty), but that didn't help.
  # tried setting boundary knots, but that doesn't seem to work either...
  
# Need to look at code I created previously for TTR and Flowers for Bats 
