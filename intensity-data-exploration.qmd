...
# See project notes for week of 2-6 June, but for now, using:
# models in glmmTMB package (ordbeta distribution) for proportions

# brms/ordbetareg spline models were prohibitively slow...

# For now, pick an example intensity-species:
i = 15 # leaf canopy fullness, dwarf birch
...
# Convert percentages to proportions and make other variables into factors
...
intdf$prop <- intdf$intensity_midpoint / 100

# Wasn't sure how to specify random effects, but based on this website:
# https://fromthebottomoftheheap.net/2017/05/04/compare-mgcv-with-glmmtmb/
# we can use an lme4 specification and that's very similar to the mgcv spec
# with bs = "re".

# Create a model with a global smooth and annual smooths with the same smoothness
# (a "GS" model in Pedersen et al. 2019). 
m1 <- glmmTMB(prop ~ s(day_of_year, k = 20) +
                s(day_of_year, fyr, k = 20, bs = "fs", m = 1) + 
                (1|individual_id),
              data = intdf, REML = TRUE, family = ordbeta)
summary(m1)

# Create new data frame for prediction (only observed days in each year)
for (yr in sort(unique(intdf$yr))) {
  ...
  if (yr == min(intdf$yr)) {
    m1_new <- data.frame(fyr = yr, day_of_year = doys)
  } else {
    m1_new <- rbind(m1_new, data.frame(fyr = yr, day_of_year = doys))
  }
}
m1_new$individual_id <- NA
m1_new$fyr <- factor(m1_new$fyr)

# Make annual predictions (averaging over individual plant effects)
m1_fit <- predict(m1,
                  m1_new,
                  se.fit = TRUE,
                  type = "link")
m1_new$fit_l <- as.numeric(m1_fit$fit)
m1_new$se <- as.numeric(m1_fit$se.fit)
m1_new <- m1_new %>%
  mutate(lcl_l = fit_l - (2 * se),
         ucl_l = fit_l + (2 * se),
         fit = exp(fit_l) / (1 + exp(fit_l)),
         lcl = exp(lcl_l) / (1 + exp(lcl_l)),
         ucl = exp(ucl_l) / (1 + exp(ucl_l)))

# Create faceted plots
ggplot(data = m1_new, aes(x = day_of_year)) +
  facet_wrap(~fyr) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.25) +
  geom_line(aes(y = fit)) +
  geom_point(data = intdf, aes(y = prop))

#TODO: Figure out how to make/plot global predictions (just the s(day_of_year) term)
  # See if emmeans or marginaleffects packages will be helpful.
#TODO: Determine whether we should use tprs (default) or cubic (cr) splines. 
  # AIC for tprs lower, but day_of_year term p-value higher (0.21 vs 0.0003).
  # Annual predictions from models look pretty similar.
#TODO: Look into model checking/diagnostics (DHARMa package?)
#TODO: Do we need to assess signficance of model components?
  # Annual smooths and/or individual random effects
```

```{r models-count-data}

# See project notes for week of 2-6 June, but for now, using:
# models in mgcv package (Gamma distribution) for numeric data (after adjusting 0 values)
...
# modeling original data with a ziGamma distribution (with glmmTMB) seems the
  # most appropriate, but get crazy SE/CIs for long periods (even when all 0s)

# For now, pick an example intensity-species:
i = 1 # breaking leaf buds, dwarf birch
...
# Create a model with a global smooth and annual smooths with the same smoothness
# (a "GS" model in Pedersen et al. 2019). 
m2 <- gam(midpoint_adj ~ s(day_of_year, k = 20) + 
            s(day_of_year, fyr, k = 10, bs = "fs", m = 1) + 
            s(individual_id, bs = "re"),
          data = intdf, 
          method = "REML", family = Gamma(link = "log"), select = TRUE,
          knots = list(day_of_year = c(0, 365)))
summary(m2)
  # Get a warning: model has repeated 1-d smooths of same variable.
gratia::draw(m2)
  #TODO: Understand this output better
  # Look at individual plants -- one seems to be an outlier
mgcv::gam.check(m2)
  # Low p-values indicate a problem...
...
m2_new$fit_l <- as.numeric(m2_fit$fit)
m2_new$se <- as.numeric(m2_fit$se.fit)
m2_new <- m2_new %>%
  mutate(lcl = exp(fit_l - (2 * se)),
         ucl = exp(fit_l + (2 * se)),
         fit = exp(fit_l))

# Create faceted plots
ggplot(data = m2_new, aes(x = day_of_year)) +
  facet_wrap(~fyr) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.25) +
  geom_line(aes(y = fit)) +
  geom_point(data = intdf, aes(y = midpoint_adj)) +
  scale_y_log10(breaks = c(0.01, 1, 10, 100, 1000, 5000), 
                labels = c("0.01", "1","10","100","1000", "5000"))
```
